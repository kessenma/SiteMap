/**
 * SiteMap devtools — local SQLite browser
 *
 * Finds the PowerSync database inside the iOS Simulator app container,
 * opens it read-only, and serves a web UI at http://localhost:4000.
 *
 * Usage:
...
const PORT = 4000;
// The DB filename from apps/mobile/src/config — DB_CONFIG.NAME + '.sqlite'
const DB_FILENAMES = ['sitemap-db.sqlite'];
// App bundle ID (from ios/SiteMap.xcodeproj — org.reactjs.native.example.SiteMap)
...
async function handleRequest(req, res) {
  ...
  if (path === '/api/debug') {
    ...
    try {
      mdfindOut = execSync(
        `mdfind -name "sitemap-db" 2>/dev/null | grep -i simulator | head -10`,
        { shell: '/bin/zsh', timeout: 5000 }
      ).toString().trim();
...
server.listen(PORT, () => {
  console.log(`\n  SiteMap devtools — SQLite browser`);
  ...
  if (dbPath) {
    ...
    console.log(`  DB not found yet — run the app in a simulator first, then refresh the browser.`);
  }
});
